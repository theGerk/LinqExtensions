...
namespace Gerk.LinqExtensions
{
	...
	public static class AsyncExtensions
	{
		...
		/// <param name="self">List that we are starting with.</param>
		/// <param name="func"><see langword="async"/> function that maps from elements of <paramref name="self"/> to an output type.</param>
		/// <param name="cancellationToken">Cancellation token that will be passed to each call of <paramref name="func"/>. Triggering it will not immidately end execution of the function.</param>
		/// <param name="concurrencyLimit">Maximum tasks to have running in parallel. A <c>0</c> will have no limit.</param>
		/// <returns></returns>
		...
		/// <param name="concurrencyLimit">Maximum tasks to have running in parallel. A <c>0</c> will have no limit.</param>
		/// <returns></returns>
		public static Task ForEachAsync<In>(this IEnumerable<In> self, Func<In, Task> action, int concurrencyLimit = 0) => self.ForEachAsync((x, _) => action(x), CancellationToken.None, concurrencyLimit);
		...
		/// <param name="self">Enumerable that we are starting with.</param>
		/// <param name="action"><see langword="async"/> function that will act on each element of <paramref name="self"/></param>
		/// <param name="cancellationToken">Cancellation token that will be passed to each call of <paramref name="action"/>. Triggering it will not immidately end execution of the function.</param>
		/// <param name="concurrencyLimit">Maximum tasks to have running in parallel. A <c>0</c> will have no limit.</param>
		/// <returns></returns>
		public static async Task ForEachAsync<In>(this IEnumerable<In> self, Func<In, CancellationToken, Task> action, CancellationToken cancellationToken, int concurrencyLimit = 0)
		{
			if (concurrencyLimit <= 0)
			{
				await Task.WhenAll(self.Select(x => action(x, cancellationToken)));
			}
			else
			{
				var inputList = self.ToList();
